...
controls whether or not to standardize the batch of observations before applying
the estimator. Standardization usually yields better results and is recommended 
...
the Hermite series based estimator sequentially, a running mean and running 
standard deviation is maintained in order to standardize the observations in an 
online manner. 
...
to calculate probability densities, cumulative probabilities and quantiles is 
presented below.
...
Hermite series based estimators can also be consistently combined. In 
particular, when standardize = FALSE, the pdf, cdf and quantile value results 
obtained from combining distinct hermite_estimators updated on subsets of a data
set are exactly equal to those obtained by constructing a single 
hermite_estimator and updating on the full data set (corresponding to the 
concatenation of the aforementioned subsets). When standardize = TRUE, the 
equivalence is no longer exact, but is accurate enough to be practically useful.
