...
hermite_est <- hermite_estimator(N=10, standardize=TRUE, 
                                 est_type = "bivariate")
for (idx in c(1:nrow(observations))) {
  hermite_est <- update_sequential(hermite_est,observations[idx,])
}
```
...
hermite_est_combined <- combine_hermite(list(hermite_est_1,hermite_est_2))
```
...
involving grouped data (as we will later illustrate).
...
num_obs <- 4000
rho <- 0.5
observations_mat <- mvtnorm::rmvnorm(n=num_obs,mean=rep(0,2),sigma = matrix(c(sig_x^2,rho*sig_x*sig_y,rho*sig_x*sig_y,sig_y^2), nrow=2,ncol=2,byrow = T))

hermite_est <- hermite_estimator(N = 20, standardize = T, est_type = "bivariate") 
...
```{r}
actual_pdf <-mvtnorm::dmvnorm(x_grid,mean=rep(0,2),sigma = matrix(c(sig_x^2,rho*sig_x*sig_y,rho*sig_x*sig_y,sig_y^2), nrow=2,ncol=2,byrow = T))

actual_cdf <- rep(NA,nrow(x_grid))
for (row_idx in 1:nrow(x_grid)) {
  actual_cdf[row_idx] <-  mvtnorm::pmvnorm(lower = c(-Inf,-Inf),upper=as.numeric(x_grid[row_idx,]),mean=rep(0,2),sigma = matrix(c(sig_x^2,rho*sig_x*sig_y,rho*sig_x*sig_y,sig_y^2), nrow=2,ncol=2,byrow = T))
}
...
Actual Spearman's rank correlation is `r round(actual_spearmans,3)`\
Estimated Spearmans's rank correlation is `r round(spear_est,3)`
...
p1+ ggtitle("Actual PDF")+ theme(legend.title = element_blank()) + p2 + ggtitle("Estimated PDF") +theme(legend.title = element_blank())+
...
p1+ ggtitle("Actual CDF")+ theme(legend.title = element_blank()) + p2 + ggtitle("Estimated CDF") +theme(legend.title = element_blank())+
  plot_layout(guides = 'collect')
```
  ...
  output$plot <- renderPlot({
    ...
    p1 <- ggplot(df_pdf) + geom_tile(aes(X, Y, fill= actual_pdf)) +
      scale_fill_gradient2(low="blue", mid="cyan", high="purple",
                           midpoint=.2,    
                           breaks=seq(0,.4,by=.1), 
                           limits=c(0,.4)) +ggtitle(paste("True Bivariate Normal Density with matched Spearman's correlation")) +
