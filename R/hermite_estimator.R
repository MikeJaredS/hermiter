...
#' subsets of a distributed data set can be consistently combined. The estimator
#' is based on the research presented in:
#' 
#' \url{https://projecteuclid.org/euclid.ejs/1488531636}, 
#' \url{https://link.springer.com/article/10.1007/s00184-020-00785-z}
#'
...
update_sequential.hermite_estimator <- function(this, x)
{
  ...
  if (this$standardize_obs==TRUE){
    if (is.na(this$exp_weight)){
      ...
      processed_vec <- standardizeInputsEW(x,this$num_obs,this$exp_weight,this$running_mean,this$running_variance)
      this$running_mean <- processed_vec[1]
      this$running_variance <- processed_vec[2]
      x <- processed_vec[3]
      if (this$num_obs < 2){
